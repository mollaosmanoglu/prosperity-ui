...
const PnlChartInner = memo(function PnlChartInner({ data: fullData, product }: { data: PnlPoint[], product: string }) {
  const { zoomedData, zoomDomain, resetZoom, chartHandlers } = useChartZoom(fullData)
  const sampled = useMemo(() => lttb(zoomedData, SAMPLE_TARGET, d => d.total), [zoomedData])
...
const PnlAllProductsInner = memo(function PnlAllProductsInner({ data: fullData, products }: { data: PnlPoint[], products: string[] }) {
  const { zoomedData, zoomDomain, resetZoom, chartHandlers } = useChartZoom(fullData)
  const sampled = useMemo(() => lttb(zoomedData, SAMPLE_TARGET, d => d.total), [zoomedData])
...
const PnlComparisonInner = memo(function PnlComparisonInner({ data: fullData, runs, product }: { data: Record<string, number>[], runs: ComparisonSeries[], product: string }) {
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <Dialog>
            ...
            <DialogContent className="!max-w-[95vw] w-full p-6">
              <div className="flex items-center gap-1.5 mb-2">
                <h3 className="text-xs font-semibold">PnL Comparison</h3>
                {zoomDomain && <button onClick={resetZoom} className="rounded-md bg-blue-50 px-2 py-0.5 text-[10px] font-medium text-blue-600 hover:bg-blue-100 transition-colors">Reset</button>}
              </div>
              <ChartZoomWrapper>{renderChart("h-[80vh]")}</ChartZoomWrapper>
            </DialogContent>
